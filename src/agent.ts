...
export function buildSystemPrompt(options?: AgentOptions): Array<{ type: "text"; text: string }> {
  ...
  // Always inject shared memory files (USER.md + MEMORY.md)
  const userMd = loadContextFile(workspace, "USER.md");
  const memoryMd = loadContextFile(workspace, "MEMORY.md");
  ...
  // Inject available skills summary (skill bodies load on-demand via file read)
